...
class NoofnyController:
    ...
    def AddListeners(self):
        ...
        try:
            ...
            masterTrackIndex = self.GetTrackIndex(masterTrack)
            ...
            self.AddMasterClipListeners(masterTrack, masterTrackIndex)
        except:
    ...
    def AddClipListeners(self, track, trackIndex, trackChannel):
        if (self._LOG_LISTENER_EVENTS):
        ...
        try:
            for clipSlotIndex in range(0, 128):
                ...
                if not (clipSlot.has_clip):
                    continue
                clip = clipSlot.clip
                clipTriggeredListener = self.CreateClipTriggeredListener(clip, clipSlotIndex, clipIndex, track, trackIndex, trackChannel)
                if not (clip.is_triggered_has_listener(clipTriggeredListener)):
                    clip.add_is_triggered_listener(clipTriggeredListener)
                clipPlayingListener = self.CreateClipPlayingListener(clip, clipSlotIndex, clipIndex, track, trackIndex, trackChannel)
                if not (clip.is_playing_has_listener(clipPlayingListener )):
                    clip.add_is_playing_listener(clipPlayingListener)
        except:
            self.logger.log("    ERROR >>> AddClipListeners track=" + str(track.name))


    # Here we tell Live which methods we want to execute when certain CLIP events are fired.
    def AddGroupClipListeners(self, track, trackIndex, trackChannel):
        if (self._LOG_LISTENER_EVENTS):
        ...
        try:
            for clipSlotIndex in range(0, self._MAX_NUMBER_OF_GROUP_TRACK_CLIPS):  
                ...
                if not (clipSlot.has_clip):
                    continue
                clipPlayingListener = self.CreateGroupClipPlayingListener(clipSlot.clip, clipSlotIndex, track, trackIndex, trackChannel)
                if not (clipSlot.clip.is_playing_has_listener(clipPlayingListener )):
                    clipSlot.clip.add_is_playing_listener(clipPlayingListener)
        except:
            self.logger.log("    ERROR >>> AddGroupClipListeners track=" + str(track.name))

    # Here we tell Live which methods we want to execute when certain CLIP events are fired.
    def AddMasterClipListeners(self, track, trackIndex):
        if (self._LOG_LISTENER_EVENTS):
        ...
        try:
            for clipSlotIndex in range(0, self._MAX_NUMBER_OF_GROUP_TRACK_CLIPS):  
                ...
                if not (clipSlot.has_clip):
                    continue
                clipPlayingListener = self.CreateMasterClipPlayingListener(clipSlot.clip, clipSlotIndex, track, trackIndex)
                if not (clipSlot.clip.is_playing_has_listener(clipPlayingListener )):
                    clipSlot.clip.add_is_playing_listener(clipPlayingListener)
        except:
    ...
    def RemoveListeners(self):
        try:
            ...
            if (self.song().metronom_has_listener(self.Song_MetronomeChanged)):
                self.song().remove_metronom_listener(self.Song_MetronomeChanged)
        ...
        try:
            ...
            self.RemoveMasterClipListeners(masterTrack, masterTrackIndex)
        except:
        ...
        for channel in range(0, 8):
            try:
                ...
                self.RemoveGroupClipListeners(groupTrack, groupTrackIndex, channel)
            except:
        ...
        for track in self.song().tracks:
            try:
                ...
                if (trackChannel == None):
                    continue    # so we skip non clip tracks.
                self.RemoveClipListeners(track, trackIndex, trackChannel)
            except:
    ...
    # Here we strip off any resident event hooks.
    def RemoveClipListeners(self, track, trackIndex, trackChannel):
        if (self._LOG_LISTENER_EVENTS):
    ...
    # Here we strip off any resident event hooks.
    def RemoveGroupClipListeners(self, track, trackIndex, trackChannel):
        if (self._LOG_LISTENER_EVENTS):
    ...
    # Here we strip off any resident event hooks.
    def RemoveMasterClipListeners(self, track, trackIndex):
        if (self._LOG_LISTENER_EVENTS):
    ...
    def Song_MetronomeChanged(self):
        if (self._LOG_LISTENER_EVENTS):
            self.logger.log("Song_MetronomeChanged ON=" + str(self.song().metronom ))
    ...
    def CreateTrackSoloListener(self, track, trackIndex, trackChannel):
        def TrackSoloChanged():
            if (self._LOG_LISTENER_EVENTS):
                self.logger.log("----------------TrackSoloChanged : track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " trackChannel=" + str(trackChannel))
            self._SOLO_STATES[trackChannel] = track.solo
            self.DisplayChannelSolos()
        return TrackSoloChanged;

    
    def CreateClipTriggeredListener(self, clip, clipSlotIndex, clipIndex, track, trackIndex, channelIndex):
        def ClipTriggered():
            if (self._LOG_LISTENER_EVENTS):
                self.logger.log("ClipTriggered > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex) + " is_triggered=" + str(clip.is_triggered))
            try:
                ...
                self.logger.log("    ERROR >>> ClipTriggered > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex) + " is_triggered=" + str(clip.is_triggered))
                return None
        return ClipTriggered;


    def CreateClipPlayingListener(self, clip, clipSlotIndex, clipIndex, track, trackIndex, channelIndex):
        def ClipPlayingChanged():
            if (self._LOG_LISTENER_EVENTS):
                self.logger.log("ClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
            try:
                clipVisible = (clipSlotIndex / self._SCENES_PER_SONG)
                if (clipVisible != self._CURRENT_SONG):
                    return None
                ...
                self.logger.log("    ERROR >>> ClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
                return None
        return ClipPlayingChanged;


    def CreateGroupClipPlayingListener(self, clip, clipIndex, track, trackIndex, channelIndex):
        def GroupClipPlayingChanged():
            if (self._LOG_LISTENER_EVENTS):
                self.logger.log("GroupClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
            try:
                self.DisplayChannelButtonsForChannel(channelIndex)
            except:
                self.logger.log("    ERROR >>> GroupClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " channelIndex=" + str(channelIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
                return None
        return GroupClipPlayingChanged;

            
    def CreateMasterClipPlayingListener(self, clip, clipIndex, track, trackIndex):
        def MasterClipPlayingChanged():
            if (self._LOG_LISTENER_EVENTS):
                self.logger.log("MasterClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
            try:
                ...
                self.logger.log("    ERROR >>> MasterClipPlayingChanged > track=" + str(track.name) + " trackIndex=" + str(trackIndex) + " clip=" + str(clip.name) + " clipIndex=" + str(clipIndex))
                return None
    ...
    def HandlePot(self, channelIndex, controllerIndex, potValue):
        ...
        try:
            if (channelIndex >= 0 and channelIndex < 8):
                ...
                # ONLY USE THESE EFFECTS ON TRACKS 1-4
                if (channelIndex >= 0 and channelIndex <= 3):
                    #playingClips = self.GetPlayingClipsForChannel(channelIndex)
                    #self.logger.log("____________HandlePot > TRACK=" + str(channelIndex) + " CONTROLLER=" + str(controllerIndex)  + " VALUE=" + str(potValue) + " CLIPS=" + str(len(playingClips)))
                    if (controllerIndex == self._CC_CHANNEL_X):
                        clip = self.song().tracks[4].clip_slots[0].clip
                        startValueRaw = (float(potValue) / float(127)) * 8
                        startValue = round(startValueRaw, 1)
                        clip.loop_start = startValueRaw
#                        if (startValue % 0.5 == 0):
#                            try:
#                                clip.loop_start = startValue
#                            except:
#                                None
#                        for clip in playingClips:
#                            startValueRaw = (float(potValue) / float(127)) * 8
#                            startValue = round(startValueRaw, 1)
#                            if (startValue % 0.5 == 0):
#                                try:
...
#                                except:
#                                    None
                    if (controllerIndex == self._CC_CHANNEL_Y):
                        clip = self.song().tracks[4].clip_slots[0].clip
                        clip.loop_end  = clip.loop_start + 0.5

                # ONLY USE THE CHOP EFFECT ON TRACKS 5-8
                elif (channelIndex >= 4 and channelIndex <= 7):
                    # CHOP - %
                    if (controllerIndex == self._CC_CHANNEL_CHOP_AMMOUNT):
                        groupTrack.devices[1].parameters[1].value = potValue
                    # CHOP - RATE
                    elif (controllerIndex == self._CC_CHANNEL_CHOP_RATE):
                        groupTrack.devices[1].parameters[2].value = potValue
                    # CHOP - TYPE
                    elif (controllerIndex == self._CC_CHANNEL_CHOP_TYPE):
                        groupTrack.devices[1].parameters[3].value = potValue
        ...
        try:
            if (buttonState == self._BUTTON_DOWN):
                ...
                if (clipIndex == 10):     # STOP BUTTON
                    self._LAST_TRIGGERED_CLIPS[channelIndex] = 0
                    for trackIndex in range(len(self.song().tracks)):
                        channel = self.GetTrackChannel(trackIndex)
                        if (channel == None):
                            continue
                        if (channel != channelIndex):
                            continue
                        selectedClipSlot = self.song().tracks[trackIndex].clip_slots[clipSlotIndex]
...
#                    self.application().view.zoom_view(nav.right, '', None)
    ...
    def GetEffectChannelClipCounts(self):
        ...
        trackFxAIndex = self.GetTrackIndex(trackFxA)
        trackFxBIndex = self.GetTrackIndex(trackFxB)
        trackFxCIndex = self.GetTrackIndex(trackFxC)
        trackFxDIndex = self.GetTrackIndex(trackFxD)
        self._CLIP_COUNT_FX_A = self.GetClipCountForTrack(trackFxAIndex)
        self._CLIP_COUNT_FX_B = self.GetClipCountForTrack(trackFxBIndex)
        self._CLIP_COUNT_FX_C = self.GetClipCountForTrack(trackFxCIndex)
        self._CLIP_COUNT_FX_D = self.GetClipCountForTrack(trackFxDIndex)
    
    
    
    # Util that returns the number of populated clip slots in the given track index.
    def GetClipCountForTrack(self, trackIndex):
        result = 0
        for clip_slot in self.song().tracks[trackIndex].clip_slots:
    ...
    def GetTrackIndex(self, track):
        try:
            trackIndex = int(track.name.split('-')[0])
            return trackIndex - 1 # we use a zero-based index
        except:
    ...
    def GetTrackChannel(self, trackIndex):
        try:
            track = self.song().tracks[trackIndex]
            if (track.has_audio_output != 1):
            ...
            self.logger.log("    ERROR >>> GetTrackChannel > trackIndex=" + str(trackIndex))
            return None
    ...
    def GetGroupTrack(self, channelIndex):
        try:
            for track in self.song().tracks:
                ...
                groupTrackName = track.name.split('-')[1]
